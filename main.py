...
def summarize_chat(
    ...
    content = "Summarize the following conversation between a chatbot and a customer in a few sentences. Use only the information from the conversation.  " + content
...
def en2ar(text):
    arabic_text = GoogleTranslator(
        source='auto', target='ar').translate(text=text)
    return arabic_text
...
def summary():
    ...
    response = summarize_chat(content)
    return jsonify({'response': response.text})
...
def summary_ar():
    data = request.get_json()
    content = data['content']
    response = arabic_summarize_chat(content)
    return jsonify({'response': response})
